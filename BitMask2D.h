...
Modified to work outside of GDAL, and only on little endian machines
*/
...
template <typename T = unsigned long long> class BitMap2D
{
  public:
    ...
    int store(storage_manager *dst)
    {
        ...
        // Assume this is a little endian machine, otherwise 
        // the storage units should be swapped before writing and then back
        // See original code in gdal/frmts/mrf
        if (_packer)
